            ...
            <div class="card mb-3">
                ...
                <div class="table-responsive text-center">
                    <table class="table table-hover table-bordered table-striped m-0">
                        ...
                        <tbody>
                            @php $no = $organisasis->firstItem(); @endphp
                            @foreach($organisasis as $key => $organisasi)
                                <tr>
                                    <td>{{ $no++ }}</td>
                                    <td>{{ $organisasi->oj_nama }}</td>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            {{ $organisasi->oj_mengeluarkan_nomor }}
                                        @endif
                                    </td> 
                                    <td>{{ $organisasi->parent ? $organisasi->parent->oj_nama : '-' }}</td>
                                    <td>
                                        <a class="btn btn-warning" href="{{ route('organisasijabatan.edit', $organisasi->oj_id) }}"><i class="fa-solid fa-pen-to-square"></i> Ubah </a>
                                        <form id="delete-form-{{ $organisasi->oj_id }}" method="POST" class="d-inline" action="{{ route('organisasijabatan.destroy', $organisasi->oj_id) }}">

                                            @csrf
                                            @method('DELETE')
                                            <button class="btn btn-danger" onclick="confirmDelete(event, '{{ $organisasi->oj_id }}' )"><i class="fa-solid fa-trash"></i> Hapus</button>
                ...
                @if ($organisasis->hasPages())
                    <div class="card-footer">
                        {{ $organisasis->links('pagination::bootstrap-5') }}
                    </div>
                @endif
            </div>
        </section>
